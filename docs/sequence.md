...
Oracleの余韻が終了閾値まで下がらない場合も、開始閾値未満の残響が60ms以上続いた後、
開始閾値の1.5倍以上へ再上昇すれば新しいonsetとして境界で分割します。
終了閾値以下の短い無音や弱い音量変動では再発音条件を有効にしません。
...
強い残響や別の音が開始閾値未満へ戻らない場合は、複数Oracleを1候補として検出する可能性があります。
