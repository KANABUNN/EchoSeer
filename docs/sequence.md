...
| 設定 | 初期値 | 用途 |
|---|---:|---|
| recognition.detection_threshold | 0.025 | 固定開始RMS。自動追従で使う開始値の上限 |
...
開始値は「環境音量の3倍」と0.002の大きい方で、detection_thresholdを上限にします。
このためVoiceMeeter側の録音音量が変わっても、周囲より明確に立ち上がるOracleを検出できます。
設定値を直接超える音は履歴を待たず、従来どおり検出します。
