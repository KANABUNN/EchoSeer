...
解析候補の音量は入力の背景レベルへ自動追従します。候補を分類する直前にも解析用コピーを
peak正規化するため、VoiceMeeterやゲーム側から小さく入力された場合も、入力音量そのものを
位置の違いとして扱いません。これらの処理は録音・保存・再生用に保持するnative元音声や
登録時に保存したoriginal.wavを書き換えません。

入力全体を単純に増幅してもOracleとBGM・銃声・スキル音の比率は改善せず、既に大きい音を
クリップさせる可能性があります。このため手動ゲイン項目は追加せず、解析側の自動追従と
正規化で機器ごとの音量差を扱います。

連続するBGMでRMSが静かな状態へ戻らない場合は、登録音に似た短い立ち上がりを最初に粗く
探します。見つけた区間だけを従来の波形相関・周波数特徴・信頼度へ渡して最終判断します。
最初の候補抽出では確定用の高精度条件を要求しすぎず、弱い候補も順位とunknownのまま残して、
二回のPASS一致を含む後段の判定で誤採用を抑えます。

開始照合は登録音の先頭400msを使用し、入力倍率とDC成分の影響を除いて20ms単位で探索します。
Liveでは将来側の照合確定を待つため、認識表示に最大約0.82秒の遅延があります。登録開始照合が
使えるとき、RMSだけで生じたLOW / REJECTEDの背景音候補はTimelineとログには残しますが、
進行中のPASSの位置には数えません。開始照合のあるLOW候補と、開始照合済みで途中切れなどの
構造上の制限が付いた候補は、Oracleを確定せずunknown位置として保持します。
Timelineの行へマウスを合わせると、RMS-onlyか開始照合済みかを確認できます。認識JSONLの
onset_detectionには粗い候補Oracle・score・marginを保存し、後の実戦調整に使えます。
...
チャンネル平均 → DC 除去 → 内部レート変換 → peak 0.95 正規化を解析用コピーで共通化し、
保存した native 元音声を読み出して現在の設定で比較します。必要に応じ帯域処理と再正規化を追加します。
