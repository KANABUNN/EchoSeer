...
表示する値は波形・スペクトル・複合の類似度（0〜1）です。正解の確率や HIGH confidence ではありません。
...
waveform_weight / spectrum_weight は Phase 5 から動作します。confidence 閾値は Phase 6 用です。
...
confidence 判定と連続検出・順序処理は後続 Phase です。
