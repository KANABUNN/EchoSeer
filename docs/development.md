...
## 2026-09-15 低音量の両PASS録音

27.093秒の実録音は20ms RMS最大0.020139で、固定検出しきい値0.025では候補0件だった。
直近1秒の低い側20%を環境音量として開始値を自動調整し、1.0秒以後の局所減衰と
再上昇で余韻を分割した。最低一致度未満の効果音はPASS位置から除外して診断記録には残す。
